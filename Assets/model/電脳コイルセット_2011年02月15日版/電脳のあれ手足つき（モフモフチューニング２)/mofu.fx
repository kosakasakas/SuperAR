...
const float FurSupecularPower = 0.9;      // 毛の光る範囲
const float FurFlowScale = float2(100,1); // 毛の流れる量
const float3 FurColor = float3(0.95, 0.95, 0.95); // 毛の色

int FurShellCount = 8; // FurShellの枚数
const float FurLength = 1/150.0;  // FurShellの幅
...
texture2D fur_tex <
   string ResourceName = "2.tga";// Fur発生テクスチャ。付属のテクスチャは服のボタン部分は黒くなっている
   int Miplevels = 1;
>;
...
float4 Fur_PS(BufferShadow_OUTPUT IN,  uniform bool useToon) : COLOR
{
 ...
 // float3 HalfVector = normalize( normalize(IN.Eye)  + -LightDirection );
    ...
    Color.rgb = lerp( TexColor, FurColor, Specular.r); // Specular.rによってTexColor -> FurColorに変化させる
...
> {
    ...
    pass Fur {
        ...
        VertexShader = compile vs_3_0 Fur_VS(true);
        PixelShader  = compile ps_3_0 Fur_PS(true);
    }
}
